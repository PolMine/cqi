...
our $CWB_available = (eval 'use CWB 2.002_102; 1') ? 1 : 0;

=item I<$ok> = B<cqi_server_available>();

Returns B<true> if a suitable CQPserver binary is installed on the local machine and
can be started with the B<cqi_server> function.

=cut

sub cqi_server_available {
  return $CWB_available;
}
